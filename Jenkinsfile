pipeline { 
  ...
  environment {
    ${params.PROJECT_NAME}DOCKERHUB = credentials("${env.PROJECT}1-dockerhub")
...
//    }
    ...
    stage('Dockerhub login') {
        steps {
            sh "sudo docker login -u ${params.PROJECT_NAME}DOCKERHUB_USR -p ${params.PROJECT_NAME}DOCKERHUB_PSW"
        }
    }
//    stage('Docker build') {
//        steps {
